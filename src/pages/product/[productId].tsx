...
const ProductPageContainer = ({ productId }: { productId: string }) => {
	...
	const { variants, defaultColorId, defaultDepth } = useMemo(() => {
		if (product) {
			if (product.isClothe) {

				const defColor = product?.options?.find(item => item?.name == "Colors")?.values[0]?.id as number;
				return { variants: product.variants.filter(variant => variant.options.find(option => option == defColor)), defaultColorId: defColor, defaultDepth: null }
			} else {

				const defDepth = product?.options?.find(item => item?.type == "depth")?.values[0]?.id as number;
				return { variants: product.variants.filter(variant => variant.options.find(option => option == defDepth)), defaultDepth: defDepth, defaultColorId: null }
			}
		}
		return { variants: null, defaultColorId: null, defaultDepth: null }
	}, [product])
	...
	const handleAddToCart = async () => {
		try {
			await addToCart({
				...
				sizeId: options.size,
				sizeTitle: product.options.find(item => item.type == "size")?.values.find(
					(size) => size.id == options.size
				)?.title as string,
        ...
        lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pb-24 lg:pt-16"
                ...
                shadow-sm duration-150 focus:border-indigo-500 focus:ring-indigo-500"
										...
										?.values.map((option) => (
											...
											>
												<Button
													...
													className={`flex border-2 text-primary-foreground ${options.size == option.id
														? "border-indigo-500"
														: null
														}
                    items-center justify-center overflow-hidden ${product.isClothe ? "h-10 w-16" : "h-16 w-36"
							...
							<Button
								type="submit"
								className={`mt-10 flex w-full items-center justify-center ${product.isInCart
